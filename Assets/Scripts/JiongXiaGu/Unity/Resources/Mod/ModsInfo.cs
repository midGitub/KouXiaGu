...
namespace JiongXiaGu.Unity.Resources
{
    ...
    public class ModsInfo
    {
        ...
        internal ModsInfo(IEnumerable<ModInfo> mods)
        {
            if (mods == null)
        ...
        internal ModsInfo(IEnumerable<ModInfo> mods, ModOrder modOrder)
        {
            if (mods == null)
                throw new ArgumentNullException(nameof(mods));
            if (modOrder == null)
        ...
        public void SetModOrder(ModOrder modOrder)
        {
            if (modOrder == null)
                throw new ArgumentNullException(nameof(modOrder));
            if (IsReadOnly)
                throw new InvalidOperationException(string.Format("[{0}]当前为只读状态;", nameof(ModsInfo)));

...
namespace KouXiaGu.Navigation
{
    ...
    public class LandformFactor : IPathfindFactor
    {
        ...
        int GetRoadCost(RoadNode node)
        {
            NavigationRoadInfo info;
            if (node.Exist() && resource.Road.TryGetValue(node.RoadType, out info))
            {
                return info.Cost;
            }
            return 0;
        ...
        bool TryGetLandformInfo(MapNode node, out NavigationLandformInfo info)
        {
            ...
            info = default(NavigationLandformInfo);
            return false;
        }

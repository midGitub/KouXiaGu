...
Shader "Water/DepthTest" {
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		//depth = saturate(depth - IN.proj.z);
		//o.Alpha = half4(depth, depth, depth, 1);
		//o.Alpha = tex2D(_CameraDepthTexture, IN.uv_WaterTex);
	...
	/*	fixed4 waterColor = tex2D(_GTex, float2(min(_Range.y, deltaDepth) / _Range.y, 1));
		half4 waterColor = tex2D(_GTex, float2(deltaDepth, deltaDepth));
		o.Alpha = waterColor;*/

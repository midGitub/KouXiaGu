...
namespace KouXiaGu.Initialization
{
    ...
    public abstract class ArchiveOperateWaiter : MonoBehaviour
    {
        protected ArchiveOperateWaiter() { }
        ...
        /// 所有操作者的数组;
        /// </summary>
        IArchiveOperate[] operaters;


        /// <summary>
        /// 正在等待的,若不存在则返回 null;
        /// </summary>
        public IArchiveOperate current
        {
        ...
        /// 开始进行等待;
        /// </summary>
        protected void StartSave(IArchiveOperate[] operaters, Archive archive)
        {
            if (IsWaiting)
                throw new ArgumentException("正在等待;");

            this.operaters = operaters;
            InitOperaters(archive);
        ...
        /// 设置并初始化所有操作者;
        /// </summary>
        void InitOperaters(Archive archive)
        {
            foreach (var operater in operaters)
            {
                operater.Save(archive);
            }
        }
        ...
        void CheckComplete()
        {
            if (operaters != null)
            {
                IArchiveOperate current = operaters[currentPointer];
        ...
        /// 当存在完成时调用,提供剩余数目;
        /// </summary>
        protected abstract void OnComplete(IArchiveOperate operater);

        /// <summary>
        /// 当存在一个任务失败时调用;
        /// </summary>
        protected abstract void OnFail(IArchiveOperate operater);
